...
def on_message(client, userdata, message):
    ...
    if message.topic == mqtt_topic_light_intensity:
        ...
        audio_path = "static/audio/RFID_tag_scanned.MP3"  # Adjust the path to your audio file
        subprocess.Popen(["mpv", "--no-video", audio_path])
        update_user_info(nuid_dec)
        check_temperature_against_threshold()

def update_user_info(nuid_dec):
    global email_sent
    ...
    conn.close()
    if user:
        # Reset email_sent if a new user is detected
        if user_info["user_id"] != nuid_dec:
            email_sent = False
        ...
        #audio_path = "static/audio/email_sent.MP3"
        #subprocess.Popen(["mpv", "--no-video", audio_path]) 
...
def reset_user_info():
    global email_sent
    email_sent = False  # Reset email flag when no user is found
    user_info = {
        "user_id": "",
        "name": "",
        "temp_threshold": default_temp_threshold,
...
def check_temperature_against_threshold():
    global email_sent
    humidity, temperature = read_dht_sensor()
    if temperature is not None:
        if temperature > float(user_info["temp_threshold"]) and not email_sent:
            send_email_notification(temperature)
            email_sent = True
        elif temperature <= float(user_info["temp_threshold"]):
            email_sent = False  # Reset email_sent when temperature drops below threshold
...
def toggle_fan():
    global fan_on
    if not fan_on:
        audio_path = "static/audio/motor_on.MP3" 
        GPIO.output(Motor1, GPIO.HIGH)
        GPIO.output(Motor2, GPIO.LOW)
        GPIO.output(Motor3, GPIO.HIGH)
        fan_on = True
    else:
        audio_path = "static/audio/motor_off.MP3" 
...
def send_email_notification(temp):
    sender_email = "bye-bye-iot@outlook.com"
    receiver_email = "bye-bye-iot@outlook.com"
    password = "iot4life"
...
# Initialize Bluetooth scan with better process management
def scan_bluetooth_devices(duration):
    try:
        ...
        time.sleep(duration / 1000)  # Convert ms to seconds and sleep

        # Stop scanning
        child.sendline('scan off')
        child.expect('Controller')  # Adjust this depending on your bluetoothctl version's output
        ...
        device_info = {mac: name for mac, name in devices}

        return device_info

    except Exception as e:
        print(f"Exception during Bluetooth scan: {e}")
        return None

# Define the endpoint to get Bluetooth device count with improved error handling
@app.route("/bluetooth-device-count", methods=["GET"])
def bluetooth_device_count():
    scan_duration = 3000  # Default scan duration in milliseconds
    device_info = scan_bluetooth_devices(scan_duration)

    if device_info is None:
        return jsonify({"error": "Failed to perform Bluetooth scan."}), 500

    # Filter devices based on some criteria, e.g., RSSI threshold, which needs additional implementation
    filtered_devices = {mac: name for mac, name in device_info.items() if some_condition}
